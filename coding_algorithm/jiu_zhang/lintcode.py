...
class BSTIterator:
    ...
    def __init__(self, root):
        self.stack = []
        self.pointer_node = root

    #@return: True if there has next node, or false
    def hasNext(self):
        return self.pointer_node is not None or len(self.stack) > 0

    #@return: return next node
    def next(self):
        while self.pointer_node is not None:
            self.stack.append(self.pointer_node)
            self.pointer_node = self.pointer_node.left
            
        self.pointer_node = self.stack.pop()
        visit_node = self.pointer_node
        self.pointer_node = self.pointer_node.right
        return visit_node
#

failed test case, but Submit was accepted.
{1, #, 2, #, 3}
1.1
1
...
class Solution:
    ...
    def closestKValues(self, root, target, k):
        ...
        if stack_upper < target:
            self.move_upper()
        else:
            self.move_lower()
        for _ in range(k):
            if not stack_upper:
                A.append(stack_lower[-1])
                self.move_lower
            elif not stack_lower:
                A.append(stack_upper[-1])
                self.move_upper()
...
test = Solution()
test.closestKValues(bst,4,6)

bst = TreeNode(1)
test = Solution()
test.closestKValues(bst,0.0,1)
